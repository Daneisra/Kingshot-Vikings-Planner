...
Check whether historical registrations still contain a legacy troop level outside T6-T16:

```bash
psql "postgresql://kingshot:change-this-postgres-password@127.0.0.1:5432/kingshot_vikings" \
  -c "SELECT id, nickname, troop_level FROM registrations WHERE troop_level < 6 OR troop_level > 16 ORDER BY troop_level DESC;"
```

The T6-T16 constraint applies immediately to new writes. Migration `2026-07-16_registration_troop_t6.sql` replaces the previous T7-T16 constraint without changing historical rows, then validates the new constraint automatically when no legacy row is outside the supported range.
